...
1. All UI solutions you create should be titled with your full name and the name of the project.
   ...
   1. The CLI tool (if you prefer it over Jupyter) greeting should mention your full name.
...
Take a look at [kpeg](https://github.com/evanphx/kpeg) or [Parselet](http://kschiess.github.io/parslet/). It's a Ruby
parser for [parser expression grammars](https://www.wikiwand.com/en/Parsing_expression_grammar) which is very similar to
...
[Sinatra](http://sinatrarb.com/) or [NYNY](http://alisnic.github.io/nyny/) will be enough) with one API
...
love story server and prints response in an easy readable way. There is a
[ruby kernel](https://github.com/SciRuby/iruby) to make it work.

Alternatively if you don't like to setup Jupiter you can design a CLI interface for your app. Note that this should be
an API client, not just a program importing your parser directly. Please make sure that this CLI tool exposes all
necessary features and probably some debug info to express your solution. Also provide examples for all key features
...
- [ ] Jupyter notebook or the CLI tool where we can play with the client and send requests to the server.
- [ ] Manual that describes how to use the start the app and the notebook (or how to use CLI tool and the set of example
...
- [ ] Add or update the code in the Jupyter notebook or adjust examples for CLI tool to test your solution.
